...
def arg_parse():
	...
	parser.add_argument("--det", dest = 'det', help = 
		"Image / Directory to store detections", default = 'Data/detections', type = str)
...
confidence = float(args.nms_thresh)
nms_thresh = float(args.nms_thresh)
start = 0 
CUDA = torch.cuda.is_available()


## Loading the class file into our program
num_classes = 80
...
try:
	...
	print("No file or directory with same {}".format(images))
	exit()
...
## Transform coordinates of images
im_dim_list = torch.index_select(im_dim_list, 0, output[:,0].long())
scaling_factor = torch.min(inp_dim/im_dim_list, 1)[0].view(-1,1)
output[:, [1,3]] -= (inp_dim - scaling_factor*im_dim_list[:,0].view(-1,1))/2
output[:, [2,4]] -= (inp_dim - scaling_factor*im_dim_list[:,1].view(-1,1))/2

## Undo scaling effectuated on the initial images
output[:,1:5] /= scaling_factor

## Clip boundary boxes with boundraies outside image edge
